...
//
//   `delivery` is chosen by the sender on the source chain. WALLET mints to the
//   recipient's Starknet address, which is a public balance. POOL hands the
//   amount to the configured delivery adapter to be filled into the recipient's
//   open note, so the position arrives private. POOL degrades to WALLET rather
//   than failing -- see the gateway.
//
...
pub struct MintMessage {
    ...
    /// Also the owner of the balance if delivery is refused and it quarantines,
    /// so it is carried even when the destination is a pool note.
